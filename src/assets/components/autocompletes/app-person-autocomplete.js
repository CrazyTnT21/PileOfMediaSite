...
export class AppPersonAutocomplete extends AppAutocomplete
{
  ...
  async* searchItems(value)
  {
    ...
    {
      const response = await get(join(API_URL, "people", "name", value), [["page", page], ["count", count]]);
      page++;
  ...
  async* loadItems()
  {
    ...
    while (page * count < total)
    {
      const response = await get(join(API_URL, "people"), [["page", page], ["count", count]]);
      page++;
      total = response.total;
      yield response.items;
    }
